        ...
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            {show && <div>
                ...
                <div className="form-outline mb-2">
                    ...
                    <small id="emailHelp" className="form-text text-muted text-center h5">Sin puntos y con guión
                        ejemplo: 9999999-9</small>
                    {errors.rut?.type === "required" && (
                        <p className="text-center errorMsg text-danger">Ingreso de Run es requerido.</p>)}
                    {errors.rut?.type === "rutFormato" && (
                        <p className="text-center errorMsg text-danger">El Formato ingresado no es válido.</p>)}
                    {errors.rut?.type === "rutValidator" && (
                        <p className="text-center errorMsg text-danger">El Run ingresado no es válido.</p>)}
                </div>
...
const DataUsuario = (props) => {
    ...
    if (!props.response.length) {
        ...
        component = <>
            ...
            <div className="text-center">
                <QRCodeCanvas className="w-50 h-50" id="qrCode"
                              value={`https://stackoverflow.com/questions/39523040/concatenating-variables-and-strings-in-react/pepe=${props.response[0].token}`}
